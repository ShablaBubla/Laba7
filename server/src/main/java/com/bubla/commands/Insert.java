...
import com.bubla.exceptions.KeyException;
...
public class Insert extends PrimeCommand {
    ...
    public String execute(String args, ServerApplication application){
        try{Product prod;
            ...
            prod = application.getNewProd();
            DataBaseManager.addProduct(prod, args);
            prods.add(args, prod);
            application.setProducts(prods);
            return "Элемент успешно добавлен";
        }
        catch (KeyException | IllegalArgumentException | SQLException e){
            return e.getMessage();
        }
        catch (ArrayIndexOutOfBoundsException e){
            return "Неверный формат ввода данных из файла";
        }
    }
